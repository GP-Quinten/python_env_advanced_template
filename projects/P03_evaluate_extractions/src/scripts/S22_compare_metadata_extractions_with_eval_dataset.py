...
@click.option(
    ...
    required=True,
    help="Path to metadata extractions JSON file from a specific model",
)
@click.option(
    "--eval_dataset_json",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    ...
    type=str,
    required=True,
    help="Path to the prompt file for LLM judge",
)
@click.option(
    "--model",
    type=str,
    required=True,
    help="Model name (e.g., small_mistral, large_mistral, gpt4o_openai)",
)
@click.option(
    "--field",
    type=str,
...
def main(
    ...
    """
    Compare metadata extractions from a model with evaluation dataset for a specific field.

    The process:
    1. Load metadata extractions from the model
    2. Load evaluation dataset with ground truth annotations
    3. For each record:
    ...
    # prompt_llm_judge = f"etc/prompts/llm_as_a_judge_annotation/compare_{field}_for_eval_dataset_creation.txt"
    ...
    logger.warning(f"Using model config: {judge_model_cfg.get('model', 'unknown')}")

    # Load metadata extractions
    with open(metadata_extractions, "r", encoding="utf-8") as f:
        extraction_records = json.load(f)
    logger.warning(
        f"Loaded {len(extraction_records)} records from metadata extractions"
    )
