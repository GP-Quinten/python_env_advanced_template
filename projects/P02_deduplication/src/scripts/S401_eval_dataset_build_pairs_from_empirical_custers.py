...
@click.option(
    ...
    type=str,
    required=True,
    help="Input parquet file path: clustered data source names with embeddings.",
)
@click.option(
    "--pairs_parquet",
    type=str,
    help="Output path where the parquet file with pairs will be saved.",
)
def main(clusters_parquet, pairs_parquet):
    """
    Main function to generate pairs of data source names based on clustering.
    """
    ...
    # Load the registry names with embeddings and clusters
    ...
    # Save df_pairs
    logging.warning("Uploading pairs ...")
    # make sure directory exists
    pairs_dir = Path(pairs_parquet).parent
