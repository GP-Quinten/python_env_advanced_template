...
def custom_score(precision, recall, beta=sqrt(1 / 2)):
...
def evaluate_stage(df_local, df_eval, cluster_col="cluster"):
    ...
    cluster_map = df_local.set_index("full_name")[cluster_col].to_dict()
    ...
    beta = sqrt(1 / 2)  # 0.707  #
    custom_score = custom_score(precision, recall, beta=beta)
        ...
        for eps2 in middle_pbar:
            ...
            for eps3 in inner_pbar:
                ...
                df_sub_final, final_time = subcluster_big_clusters(
                    df_sub_noise, eps3=eps3, min_samples=2, size_threshold=5
                )
                # Build "Final_Cluster"
