...
@click.option(
    ...
    required=True,
    multiple=True,
    help="Paths to LLM records JSONL files for cost estimation.",
)
@click.option(
...
def main(
    ...
    models = ["small_mistral", "large_mistral"]
    regular_fields = [
        "registry_name",
        "geographical_area",
        "medical_condition",
    ...
    # Calculate costs for regular fields
    regular_costs = {}
    for file in llm_records_jsonls:
        path = Path(file)
        model = path.parent.name
        cost_data = estimate_cost_from_records(file, model)
        regular_costs[model] = cost_data
        logger.warning(
            f"Calculated costs for {model} regular fields: {cost_data['total_cost']:.4f} euros ({cost_data['num_records']} records)"
        )
    ...
    rows = []
    # Header row (empty, empty, empty, SMALL mistral x6, empty, LARGE mistral x6)
    header = [
        "",
        "",
        "",
    ...
    reg_values_row = ["", "", ""]
    # SMALL mistral metrics
    for model in ["small_mistral", "large_mistral"]:
    ...
    # Rows for each field
    total_costs = {"small_mistral": 0, "large_mistral": 0}
    for field in [
        "registry_name",
        "geographical_area",
        "medical_condition",
        ...
        for model in ["small_mistral", "large_mistral"]:
            ...
            if metrics_key in perf_metrics_data:
                ...
                cost_data = regular_costs.get(model, None)
                cost_per_record = (
                    cost_data["total_cost"] / cost_data["num_records"]
                    if cost_data and cost_data["num_records"] > 0
                    else 0
    ...
    # For other fields, columns are:
    # ['', '', '', 'cost on 220K', 'cost for publications left', 'accuracy', 'counts correct answers', 'Details', '', ...]
    # But for pandas, just use default numbering, as the template is visually matched by position
    ...
    with pd.ExcelWriter(output_excel, engine="openpyxl") as wb:
        ...
        # For the second line, use rich text if possible (openpyxl does not support rich text in a single cell, so use workaround: set all to bold, but add a comment for manual adjustment if needed)
        # cell.comment = Comment("Set (73% publications are registry related) to size 8 italic", "Copilot")

        # 3. Merge D1:H1 and J1:N1
        worksheet.merge_cells("D1:H1")
        worksheet["D1"].value = "SMALL mistral"
        worksheet.merge_cells("J1:N1")
        worksheet["J1"].value = "LARGE mistral"
        ...
        # 8. Registry-related metrics placement is already handled in the row construction
        # 9. Costs are already formatted in euros

        # Make all header cells bold (A2, B2, C2 to C8, D1, J1, D2 to H2, J2 to N2)
        # A2, B2 (already merged cells)
