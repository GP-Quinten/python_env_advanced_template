...
@click.option(
    ...
    required=True,
    help="Path to extraction results JSON file from R22.",
)
@click.option(
    "--model",
    type=str,
...
def main(extraction_results, model, field, output_json):
    """
    Compute and save performance metrics from extraction results.

    This script:
    1. Loads the extraction comparison results
    ...
    logger.warning(f"Overall performance metrics:")
