...
prompt_file_path = Path("etc/prompts/prompt_pairing_assignment_v2.txt")
# prompt_file_path = Path("etc/prompts/compare_registry_name.txt")
...
async def run_async_inference(test_pairs: pd.DataFrame, prompts: list) -> pd.Series:
    """
    ...
    backend = MistralAsyncBackend(api_key=api_key, cache_storage=TmpCacheStorage())
    start_time = time.time()

    # Make inferences
    logging.warning(f"Making inferences for {len(test_pairs)} pairs")
    results = []
    num_requests = len(prompts)
    pbar = tqdm(total=num_requests, desc="Processing async batch")
    async for result in backend.infer_many(prompts, model_config):
        parsed_result = backend._parse_response(result)
    ...
    end_time = time.time()
    logging.warning(
        f"Processed {num_requests} requests in {end_time - start_time:.2f} seconds"
    )
    return results
...
def main():
    ...
    # Load df_pairs from S3
    df_pairs = load_file_from_s3(
    ...
    logging.warning("Loaded df_pairs from S3")
    ...
    # logging.warning("Starting asynchronous inference on 100 test pairs...")
    # loop = asyncio.get_event_loop()
    # results = loop.run_until_complete(run_async_inference(df_pairs, prompts))
    # logging.warning("Asynchronous inference complete for test sample")
    ...
    # ----- POST-PROCESSING -----
    # each element contains {"explanation", "final_decision", "custom_id"}'
    ...
    # # For now, we update the test sample back into df_pairs for demonstration (in practice, use the batch result)
    # df_pairs.loc[df_pairs.index, "final_label"] = df_pairs["final_label"]
    ...
    # ----- SAVE DATA -----
    # Save final pairs dataset to S3
    ...
    logging.warning("Uploading final pairs to S3...")
    upload_file_to_s3(df_pairs, config.BUCKET_NAME_DEV, folder_path, file_name)
    logging.warning("Upload to S3 completed.")

    # Save final pairs as Excel file locally, with header
    local_excel_path = Path("data/4_final_pairs/final_pairs_v3.xlsx")
    local_excel_path.parent.mkdir(parents=True, exist_ok=True)
    df_pairs.to_excel(local_excel_path, index=False, header=True)
    logging.warning(f"Final testing pairs saved locally to {local_excel_path}")

    # save df_pairs_pivot as Excel file locally, with header
    local_excel_path_pivot = Path("data/4_final_pairs/category_switch_v3.xlsx")
    local_excel_path_pivot.parent.mkdir(parents=True, exist_ok=True)
    df_pairs_pivot.to_excel(local_excel_path_pivot, index=True, header=True)
