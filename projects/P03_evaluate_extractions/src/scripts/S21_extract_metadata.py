...
def parse_mistral_response(response):
    """
    Parse Mistral response to extract the metadata fields.
    """
    try:
...
def parse_openai_response(response):
    """
    Parse OpenAI response to extract the metadata fields.
    """
    try:
...
@click.option(
    ...
    type=str,
    required=True,
    help="Model to use for inference (e.g., 'mistral', 'openai')",
)
@click.option(
    "--output_json",
    type=str,
    required=True,
    help="Path to output JSON file with metadata extractions",
)
@click.option(
...
def main(
    ...
    """Extract metadata from PubMed dataset using the specified model."""
    ...
    prompts = []
    for rec in records:  # Limit to first 5 records for testing
    ...
    logger.warning(f"Starting batch inference with {model_name}...")
    ...
    logger.warning(f"Saved metadata extractions to {output_json}")
    ...
    logger.warning(
        f"Saved {len(prompt_response_records)} records to {output_records_jsonl}"
    )
