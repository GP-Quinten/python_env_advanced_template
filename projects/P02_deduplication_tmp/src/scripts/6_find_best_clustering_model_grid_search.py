...
def main():
    ...
    eps1_values = [
        0.325
    ]  # np.arange(0.305, 0.360001, 0.005)  # np.arange(0.25, 0.450001, 0.005) # for global
    eps2_values = [
        0.40,
        # 0.45,
        # 0.50,
    ]  # np.arange(0.30, 0.500001, 0.05)  # [0.35]  # for noise
    eps3_values = [0.275]  # np.arange(0.265, 0.280001, 0.005)  # for big clusters
    ...
    # ------------- SAVE TO DISK / S3 -------------
    save_path = "data/6_find_best_model/grid_search_results_v4_all.xlsx"
    results_df.to_excel(save_path, index=False)
    logging.warning(f"Nested grid search finished. Results saved to '{save_path}'.")

    upload_file_to_s3(
        results_df,
        bucket_name=config.BUCKET_NAME_DEV,
        folder_path="registry_data_catalog_experiments/task_1_deduplication/grid_search_results/",
        file_name="grid_search_results_v4_all.parquet",
    )
    logging.warning("Results uploaded to S3 as 'grid_search_results_v4_all.parquet'.")
